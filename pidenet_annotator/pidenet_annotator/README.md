# PIDENet 抓取位姿离线标注算法复现（Phase 1）

本仓库实现了论文 *PIDENet: Physics-Informed Dense Voting for End-to-End Grasping* 3.2 节所描述的**离线抓取位姿标注算法**（Section 3.2.1 – 3.2.5），高斯扩散（3.2.6）按你的要求暂不包含，留待 Phase 2。

**当前阶段（Phase 1）的产出**：给定一个物体的 CAD 模型（`.ply`），在物体自身坐标系中生成一组**排序好的候选抓取位姿**，每个位姿包含 `center_pkm / approach_vector v / orientation_vector u / gripper_width / Q_score` 等字段，同时提供三种可视化。

Phase 2（把候选投影到每帧相机坐标系、生成图片级 label）等你确认 Phase 1 的结果之后再做。
...
如果发现 bug 或有超参数不合理，把 `outputs/` 打包发给我即可诊断。
