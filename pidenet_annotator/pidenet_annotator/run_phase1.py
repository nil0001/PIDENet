...
def main():
    ...
    parser.add_argument('--min-Q', type=float, default=0.1,
                        help='Discard candidates with Q_score below this '
                             'threshold from every output (terminal print, '
                             'YAML, both visualizations). Set to 0.0 to keep '
                             'everything, including infeasible ones. Default: 0.1.')
    ...
    for ply_path, tag in resolve_ply_paths(args):
        ...
        n_before = len(res['candidates_final'])
        res['candidates_final'] = [c for c in res['candidates_final']
                                    if c['Q'] >= args.min_Q]
        n_after = len(res['candidates_final'])
        if args.min_Q > 0:
            print(f'  candidates final: {n_after}  '
                  f'(dropped {n_before - n_after} below Q={args.min_Q})')
        else:
            print(f'  candidates final: {n_after}')
