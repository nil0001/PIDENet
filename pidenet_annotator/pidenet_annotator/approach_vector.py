...
class SurfaceQuery:
    ...
    def __init__(self, mesh, n_samples=40000, seed=0):
        # trimesh.sample.sample_surface picks up numpy's global RNG state,
        # so seeding numpy here guarantees reproducible KNN neighborhoods
        # (and therefore reproducible PCA-derived approach vectors) across
        # runs, which matters when the caller relies on the exact ranking.
